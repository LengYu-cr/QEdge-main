...
public void sendCard(String str,String str2,int mtype) {
    String qr="";
    if(mtype==1) qr=getUidFromUin(str);
    else if(mtype==2) qr=str;
    Contact contact = new Contact(mtype, qr, "");
    card(contact,str2);
}
public void sendMsg(String str,String str2,int mtype) {
    String qr="";
    if(mtype==1) qr=getUidFromUin(str);
    else if(mtype==2) qr=str;
    Contact contact = new Contact(mtype, qr, "");
...
public void sendPtt(String str,String str2,int mtype) {
    String qr="";
    if(mtype==1) qr=getUidFromUin(str);
    else if(mtype==2) qr=str;
    Contact contact = new Contact(mtype, qr, "");
...
public void sendVideo(String str,String str2,int mtype) {
    String qr="";
    if(mtype==1) qr=getUidFromUin(str);
    else if(mtype==2) qr=str;
    Contact contact = new Contact(mtype, qr, "");
...
public void sendPic(String str,String str2,int mtype) {
    String qr="";
    if(mtype==1) qr=getUidFromUin(str);
    else if(mtype==2) qr=str;
    Contact contact = new Contact(mtype, qr, "");
    send(contact,"[pic=" + str2 + "]");
}
...
public void sendReply(Object data,String menu) {
    //sendReply(data.GroupUin,data,menu);回复
}
public Activity getNowActivity() {
    return getThreadActivity();
}
...
public void sendZan(String str, int j) {
    return zan(str,j);
}
...
public List getGroupMemberList(String qun) {
    List mapList = new ArrayList();
    List originListMapList = members(qun, false);
    for (Object originListMap : originListMapList) {
        HashMap ListMap = new HashMap();
        ListMap.put("uin", originListMap.memberuin);
        ListMap.put("uinName", originListMap.friendnick);
        ListMap.put("uinLevel", originListMap.realLevel);
        ListMap.put("lastActiveTime", originListMap.last_active_time);
        ListMap.put("joinGroupTime", originListMap.join_time);
        mapList.add(ListMap);
    }
    return mapList;
}
public List getProhibitList(String qun) {
    List mapList = new ArrayList();
    List originListMapList = members(qun, false);
    long now = System.currentTimeMillis() / 1000;
    for (Object originListMap : originListMapList) {
        if (originListMap.gagTimeStamp > now) {
            HashMap listMap = new HashMap();
            listMap.put("user", originListMap.memberuin);
            listMap.put("userName", originListMap.friendnick);
            listMap.put("endTime", originListMap.gagTimeStamp);
            long duration = originListMap.gagTimeStamp - now;
            listMap.put("time", duration);
            mapList.add(listMap);
        }
    }
    return mapList;
...
public List getGroupList() {
    List mapList = new ArrayList();
    List originListMapList = groups();
    for (Object originListMap : originListMapList) {
        HashMap ListMap = new HashMap();
        ListMap.put("group", originListMap.group);
        ListMap.put("groupName", originListMap.groupName);
        ListMap.put("groupOwner", originListMap.groupOwner);
        ListMap.put("groupAndName", originListMap.groupAndName);
        ListMap.put("encryptGroup", originListMap.encryptGroup);
        mapList.add(ListMap);
    }
    return mapList;
}
...
String QQPath=LocalPath+"Android/media/"+QQ_PackageName+"/.模了个块/Plugin/";
...
public void lengyu520(String qun, String uin, int type)
{

    if(type == 1){
        qun=getUinFromUid(qun);
    }
...
public void wyzz(String qun,String uin,int type) {
    payList();
}
public void music(String qun,String uin,int type) {
    if(type == 1){
        qun=getUinFromUid(qun);
    }
    searchMusic(qun,type);
}
public void tzjb(String qun,String uin,int type) {
    本地脚本();
}

public void kgsz(String qun,String uin,int type) {
    if(type == 1){
        qun=getUinFromUid(qun);
    }
    kgsz(qun,type);
                    ...
                    public void run() {
                        ...
                        ThisActivity.runOnUiThread(new Runnable() {
                            public void run() {
                                ...
                                statsText.setText("共 " + functionCount + " 个功能");
                            }
                        }
                        );
