...
public String getFriendRKey() {
    return CookieTool.INSTANCE.getGroupRKey();
}

public String getGroupRKey() {
    return CookieTool.INSTANCE.getFriendRKey();
}
...
public void sendPtt(String qun,String url,int mtype) {
     String jb = "语音"+User(3);
     String file = ColdRainPath + "/语音/" + jb + ".amr";
     String sp = DownloadToFile(url, file);
     if(sp.equals("成功"))
     {
         MsgTool.INSTANCE.sendPtt(qun,file,mtype);
         sc(file);
     }
     else
     {
         sendMsg(qun, "语音链接下载" + sp,mtype);
     }
}
...
String QQPath=LocalPath+"Android/data/"+QQ_PackageName+"/QFun/"+myUin+"/plugin/";
...
List<List<String>> functionList = new ArrayList<List<String>>();
