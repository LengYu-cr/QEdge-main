...
try {
    ...
    $uploadsDir = $projectRoot . '/uploads/plugins/';
    ...
    if ($action === 'approve') {
        ...
        $pdo->commit();
        logMessage("用户 {$userQQ} 审核通过插件 {$plugin['plugin_name']} (could_id: {$couldId})", 'INFO');
        ...
        if (!file_exists($currentFilePath)) {
            ...
            if (file_exists($pendingPath)) {
                ...
                $approvedPath = $uploadsDir . 'approved/' . $filename;
                if (file_exists($approvedPath)) {
                    $currentFilePath = $approvedPath;
                } else {
                    throw new Exception('文件不存在');
                }
            }
        ...
        $pdo->commit();
        logMessage("用户 {$userQQ} 审核拒绝插件 {$plugin['plugin_name']} (could_id: {$couldId}) 原因: {$rejectReason}", 'INFO');
        ...
        $pdo->commit();
        logMessage("用户 {$userQQ} 下架脚本 {$plugin['plugin_name']} (could_id: {$couldId})", 'INFO');
