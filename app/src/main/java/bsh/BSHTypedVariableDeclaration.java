...
class BSHTypedVariableDeclaration extends SimpleNode {
    ...
    public Object eval(CallStack callstack, Interpreter interpreter)
        ...
        try {
            ...
            BSHType typeNode = getTypeNode();
            Class<?> type;
            if (typeNode.isVarType()) {
                type = null;
            } else {
                type = typeNode.getType(callstack, interpreter);
            }
            ...
            for (int i = 0; i < bvda.length; i++) {
                ...
                if (typeNode.isVarType()) {
                    if (value == null || value == Primitive.NULL || value == Primitive.VOID) {
                        throw new EvalError(
                                "Cannot infer type for 'var' variable '" + dec.name + "' with null or void initializer",
                                this, callstack);
                    }
                    if (value instanceof Primitive) {
                        inferredType = ((Primitive) value).getType();
                    } else {
                        inferredType = value.getClass();
                    }
                }
                ...
                try {
                    ...
                    if (null != lhs && null != lhs.field) {
                        Variable var = new Variable(dec.name, inferredType, lhs);
                        ...
                        if (interpreter.getStrictJava()
                                ...
                                && inferredType != null)
                            value = Primitive.castNumberStrictJava(inferredType,
                                    ((Primitive) value).numberValue());
                        namespace.setTypedVariable(
                                dec.name, inferredType, value, modifiers);
