...
class BSHType extends SimpleNode implements BshClassManager.Listener {
    ...
    public boolean isVarType() {
        if (isVarType) return true;
        if (jjtGetNumChildren() > 0) {
            Node child = jjtGetChild(0);
            if (child instanceof BSHAmbiguousName) {
                String name = ((BSHAmbiguousName) child).text;
                if (name != null && name.equals("var") && arrayDims == 0) {
                    isVarType = true;
                    return true;
                }
            }
    ...
    public Class<?> getType(CallStack callstack, Interpreter interpreter)
            throws EvalError {
        if (isVarType)
            throw new EvalError("Cannot resolve 'var' type without initializer", this, callstack);
