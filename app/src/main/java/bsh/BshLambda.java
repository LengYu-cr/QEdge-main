...
public abstract class BshLambda {
    ...
    protected <T> T convertTo(Class<T> functionalInterface) throws UtilEvalError {
        ...
        if (this instanceof BshLambdaFromLambdaExpression) {
            Method fiMethod = methodFromFI(functionalInterface);
            ((BshLambdaFromLambdaExpression) this).inferTypesFrom(fiMethod);
        }
    ...
    private static class BshLambdaFromLambdaExpression extends BshLambda {
        ...
        protected void inferTypesFrom(Method interfaceMethod) {
            Class<?>[] methodParams = interfaceMethod.getParameterTypes();
            if (methodParams.length != this.paramsTypes.length) return;
            boolean needInfer = false;
            for (Class<?> t : this.paramsTypes) {
                if (t == null) { needInfer = true; break; }
            }
            if (!needInfer) return;
            Class<?>[] newTypes = new Class<?>[this.paramsTypes.length];
            for (int i = 0; i < this.paramsTypes.length; i++) {
                newTypes[i] = this.paramsTypes[i] != null ? this.paramsTypes[i] : methodParams[i];
            }
            this.paramsTypes = newTypes;
        }
